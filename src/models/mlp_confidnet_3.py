...
def train(model,epoch, best_ape):
    ...
    print(f'Test Set: Avg. Loss: {test_loss:.4f}, Accuracy: {correct}/{len(test_loader.dataset)} ({(100.*correct/len(train_loader.dataset)):.2f}%) ')
    ...
    if ap_errors>best_ape:
        ...
        best_ape = ap_errors
        torch.save(model.state_dict(), f"../saved_models/mlp_confidence_3_test.pt")
    return best_ape
...
if __name__ == '__main__':
    ...
    if args.train:
        best_ape = 0
        #model.load_state_dict(torch.load("../saved_models/mlp_3_resume.pt"), strict=False)
        #freeze_weights(model)
        #disable_drop(model)
    ...
    if args.test:
        model.load_state_dict(torch.load('../saved_models/mlp_confidence_3_test.pt'))
        test(model)
